...
export default function ToolPage() {
  ...
  const databasesRef = useRef<{ idiomDatabase: any[]; aiPhraseMap: any[]; aiSuggestions: any[]; lexicalDatabases: Record<string, any[]> } | null>(null);
  ...
  const loadDatabases = useCallback(async () => {
    ...
    // Advise-only tier (register suggestions): informal-but-acceptable phrasings
    // the worker never auto-edits — it only surfaces a "consider ..." note.
    let aiSuggestionsNext: any[] = [];
    try {
      const res = await fetch("/ai-suggestions.json");
      if (res.ok) aiSuggestionsNext = (await res.json()) || [];
    } catch {}

    const dbs = { idiomDatabase: idiomDatabaseNext, aiPhraseMap: aiPhraseMapNext, aiSuggestions: aiSuggestionsNext, lexicalDatabases: lexicalDatabasesNext };
  ...
  const handleTransform = async () => {
    ...
    try {
      ...
      const response = await transformText(
        ...
        "auto",
        { idiomDatabase: dbs.idiomDatabase, aiPhraseMap: dbs.aiPhraseMap, aiSuggestions: dbs.aiSuggestions, lexicalDatabases: dbs.lexicalDatabases },
        token || undefined
      );
      setResult(response);
                ...
                <div className="flex items-center gap-1">
                  {([
                    ...
                    >
                      ...
                      {key === "notes" && (result?.suggestions?.length || result?.registerNotes?.length) > 0 && (
                        <span className="px-1.5 py-0.2 bg-blue-500/30 text-blue-300 text-[10px] font-bold rounded-full border border-blue-400/30">{(result.suggestions?.length || 0) + (result.registerNotes?.length || 0)}</span>
                      )}
                ...
                <div className="space-y-5 flex-1 flex flex-col">
                  ...
                  {outputViewMode === "notes" && (
                    <div className="p-4 bg-black/30 border border-white/10 rounded-2xl flex-1 overflow-y-auto">
                      ...
                      {result.registerNotes && result.registerNotes.length > 0 && (
                        <div className="p-3 mb-3 border border-violet-500/25 bg-violet-500/5 rounded-xl">
                          <p className="text-[10px] uppercase font-bold tracking-wider text-violet-300 mb-1.5 flex items-center gap-1.5">
                            <PenLine className="w-3 h-3" /> Register suggestions — kept as written
                          </p>
                          <div className="space-y-1.5">
                            {result.registerNotes.map((rn, i) => (
                              <p key={i} className="text-xs text-violet-100/90 leading-relaxed">{rn}</p>
                            ))}
                          </div>
                        </div>
                      )}
