...
export default function App() {
  ...
  // --- Secret bypass for testing (remove later) ---
  const urlParams = new URLSearchParams(window.location.search);
  const bypass = urlParams.get('bypass') === 'ali2026';
  
  if (!bypass) {
    // Normal limit checks (active for all users except when bypass is present)
    if (!inputText.trim()) {
    ...
    console.log("Bypass active: limits disabled");
  }
  ...
  try {
    ...
    // Update usage count only when bypass is NOT active
    if (!bypass) {
      const today = new Date().toDateString();
      const stored = localStorage.getItem('nativewrite_usage');
      let newCount = 1;
  ...
  } // end of bypass else block

  // The rest of your original handleTransform code continues here (setIsLoading(true), try/catch, etc.) 
    if (!inputText.trim()) {
      toast.error("Please enter some text to transform.");
      return;
    }
